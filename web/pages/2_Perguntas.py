from __future__ import annotations

import re
import streamlit as st

from web.rag_core import (
    get_retrievers,
    get_llm,
    montar_prompt,
    formatar_resposta,
    get_roles
)


st.set_page_config(page_title="Perguntas", page_icon="❓", layout="wide")

st.title("❓ Perguntas sobre Vinhos")
st.caption("RAG em duas etapas: contexto (PDFs) → produtos (CSVs)")

with st.sidebar:
    st.subheader("Configurações")
    model_name = st.text_input("Modelo (Ollama)", value="mistral:instruct")
    k_desc = st.number_input("Top-K contexto (PDFs)", 1, 10, 3)
    k_match = st.number_input("Top-K produtos (CSVs)", 1, 10, 3)
    debug = st.checkbox("Modo debug", value=False, help="Exibe documentos recuperados, prompts e respostas intermediárias")
    st.divider()
    if st.button("Recriar índices", help="Refaz o cache dos vetores (faça se adicionar/alterar arquivos)"):
        # Limpa caches de recursos
        st.cache_resource.clear()
        st.success("Índices recriados. Volte a executar a pergunta.")


pergunta = st.text_input("Sua pergunta", placeholder="Ex.: Qual vinho combinar com carne de porco assada?")
executar = st.button("Executar")

if executar and pergunta.strip():
    retriever_desc, retriever_match = get_retrievers(k_description=int(k_desc), k_matches=int(k_match))
    llm = get_llm(model_name=model_name)

    # Etapa 1: PDFs → descrição
    query_1 = pergunta
    docs_1 = retriever_desc.get_relevant_documents(query_1)
    contexto_1 = ". ".join([d.page_content for d in docs_1])
    role_1 = get_roles()[0]
    prompt_1 = montar_prompt(role_1, contexto_1, pergunta)
    resposta_1 = llm.invoke(prompt_1)

    if debug:
        with st.expander("Etapa 1 - Documentos recuperados (PDFs)", expanded=False):
            for d in docs_1:
                meta = d.metadata
                st.markdown(f"- {meta.get('source', '?')} | pág {meta.get('page', '?')}")
                st.write(d.page_content[:600] + ("..." if len(d.page_content) > 600 else ""))
        with st.expander("Etapa 1 - Prompt (com role)", expanded=False):
            st.code(prompt_1)
        with st.expander("Etapa 1 - Resposta", expanded=False):
            st.write(formatar_resposta(resposta_1))

    # Etapa 2: CSVs → recomendação final
    query_2 = f"query: {resposta_1}"
    docs_2 = retriever_match.get_relevant_documents(query_2)
    # Remover prefixos tipo "campo: valor" dos CSVs, como no notebook
    import re as _re
    contexto_2 = ". ".join([_re.sub(r"\b\w+:\s*", "", d.page_content) for d in docs_2])
    role_2 = (
        "Você é um sommelier experiente em um restaurante 5 estrelas. Use o contexto com vinhos reais (nome, preço, descrição) "
        "para recomendar 1 a 3 rótulos ao cliente, explicando por que combinam com o prato e o preço. Seja educado e direto."
    )
    prompt_2 = montar_prompt(role_2, contexto_2, pergunta)
    resposta_final = llm.invoke(prompt_2)

    if debug:
        with st.expander("Etapa 2 - Documentos recuperados (CSVs)", expanded=False):
            for d in docs_2:
                meta = d.metadata
                st.markdown(f"- {meta.get('source', '?')} | linha {meta.get('row', '?')}")
                st.write(d.page_content[:600] + ("..." if len(d.page_content) > 600 else ""))
        with st.expander("Etapa 2 - Prompt (com role)", expanded=False):
            st.code(prompt_2)

    st.subheader("Resposta")
    st.write(formatar_resposta(resposta_final))


